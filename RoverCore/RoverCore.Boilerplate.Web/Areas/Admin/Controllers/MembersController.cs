...
namespace RoverCore.Boilerplate.Web.Areas.Admin.Controllers
{
    ...
    public class MembersController : BaseController<MembersController>
    {
        ...
        public async Task<IActionResult> Create([Bind("MemberId,FirstName,LastName,Email,Password,PasswordSalt")] Member member)
        {
            _breadcrumbs.StartAtAction("Dashboard", "Index", "Home", new { Area = "Dashboard" })
            .ThenAction("Manage Member", "Index", "MembersController", new { Area = "Admin" })
            .Then("Create Member");     
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("MemberId,FirstName,LastName,Email,Password,PasswordSalt")] Member member)
        {
            _breadcrumbs.StartAtAction("Dashboard", "Index", "Home", new { Area = "Dashboard" })
            .ThenAction("Manage Member", "Index", "MembersController", new { Area = "Admin" })
            .Then("Edit Member");  
            ...
            model.PasswordSalt = member.PasswordSalt;
        ...
        public async Task<IActionResult> GetMember(DtRequest request)
        {
            try
            {
                ...
                {
                    records = records.Where(m => m.FirstName.Contains(searchValue)
                                || m.LastName.Contains(searchValue)
                                || m.Email.Contains(searchValue)
                                || m.Password.Contains(searchValue)
                                || m.PasswordSalt.Contains(searchValue));
                }
